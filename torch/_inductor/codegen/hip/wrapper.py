...
from ..codecache import HipKernelParamCache
    ...
    def codegen(
        ...
        if V.graph.cpp_wrapper:
            ...
            if V.graph.aot_mode:
                ...
                # HIPStreamGuard sets the stream and the device.
                if self.last_seen_device_guard_index is None:
                    if config.aot_inductor.abi_compatible:
                        code.writeline(
                            "AOTIHipStreamGuard stream_guard(stream, this->device_idx_);"
                        )
                    else:
                        code.writeline(
                            "at::hip::HIPStreamGuard stream_guard("
                            + "at::cuda::getStreamFromExternal(stream, this->device_idx_));"
                        )
                else:
                    assert (
                        self.last_seen_device_guard_index == self.device_idx
                    ), "AOTInductor only supports running on one HIP device"
            else:
                if self.last_seen_device_guard_index is None:
                    code.writeline(
...
class WrapperCodeGen(CodeGen):
    ...
    def write_header(self) -> None:
        self.header.splice(
            f"""
                ...
                empty_strided_hip = torch._C._dynamo.guards._empty_strided_hip
    ...
    def generate_kernel_call(
        ...
        triton: Defines whether the GPU backend uses Triton for codegen.
                Otherwise it uses the HIP language for codegen.
...
class CppWrapperCodeGen(WrapperCodeGen):
    ...
    def codegen_model_kernels(self):
        ...
        for kernel in declare_kernel:
            self.prefix.writeline(f"    hipFunction_t {kernel}{{nullptr}};")
        self.prefix.writeline("};")
        self.prefix.writeline("}  // namespace")
    ...
    def generate_user_defined_triton_kernel(self, kernel_name, grid, configs, args):
        ...
        if len(grid) == 1:
            ...
            meta = HipKernelParamCache.get(kernel_name)
    ...
    def make_allocation(
        ...
        if device.type == "cuda":
            return (
                f"at::Tensor {name} = at::detail::empty_strided_hip("
                f"{size}, {stride}, {dtype_code}, c10::DeviceType::HIP);"
            )
        return (
...
class HipWrapperCodeGen(CppWrapperCodeGen):
    """
    Generates cpp wrapper for running on GPU and calls HIP kernels
    """
    ...
    def write_header(self):
        ...
        self.header.splice(
            ...
            static inline hipFunction_t loadKernel(
                ...
                hipModule_t mod;
                hipFunction_t func;
                HIP_DRIVER_CHECK(hipModuleLoad(&mod, filePath.c_str()));
                HIP_DRIVER_CHECK(hipModuleGetFunction(&func, mod, funcName.c_str()));
                if (sharedMemBytes > 0) {
                    HIP_DRIVER_CHECK(hipFuncSetAttribute(
                        func,
    ...
    def write_get_raw_stream(self, index):
        name = f"stream{index}"
        self.writeline(
            f"hipStream_t {name} = at::hip::getCurrentHIPStream({index});"
        )
        return name
    ...
    def generate_default_grid(self, name: str, grid: List[Any], cuda: bool = True):
        """
        Generate grid configs for launching a HIP kernel using the grid
        ...
        params = HipKernelParamCache.get(name)
        assert (
            params is not None
        ), f"cuda kernel parameters for {name} should already exist at this moment, only found {HipKernelParamCache.get_keys()}"
        block_cfg = {
    ...
    def generate_kernel_call(
        ...
        if not cuda:
            # Even in HipWrapperCodeGen, we may see cpp kernels
            return super().generate_kernel_call(
                name, call_args, grid, device_index, cuda, triton, arg_types
            )
