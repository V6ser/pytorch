...
#include <ATen\hip\cub_definitions.cuh>
...
#define CUB_NS_PREFIX namespace at_hip_detail {
#define CUB_NS_POSTFIX }
#define CUB_NS_QUALIFIER ::at_hip_detail::cub
#include <hipcub/hipcub.hpp>
...
#if !defined(USE_ROCM)
namespace at::native {
namespace cub = ::at_hip_detail::cub;
} // namespace at::native
#endif
...
inline void segmented_sort_pairs(
  ...
  if (descending) {
    CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceSegmentedRadixSort::SortPairsDescending,
      keys_in_, keys_out_, values_in, values_out,
      num_elements, num_segments, begin_offsets, end_offsets,
      begin_bit, end_bit, c10::hip::getCurrentHIPStream());
  } else {
    CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceSegmentedRadixSort::SortPairs,
      keys_in_, keys_out_, values_in, values_out,
      num_elements, num_segments, begin_offsets, end_offsets,
      begin_bit, end_bit, c10::hip::getCurrentHIPStream());
  }
  ...
  NumSelectedIteratorT num_selected, int64_t num_input_items)
{
  ...
  CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceSelect::UniqueByKey,
    keys_in, values_in, keys_out_, values_out, num_selected, num_input_items, c10::hip::getCurrentHIPStream());
}
#endif
...
inline void inclusive_scan(InputIteratorT input, OutputIteratorT output, ScanOpT scan_op, int64_t num_items) {
  ...
  CUB_WRAPPER(NO_ROCM(detail)::hiphipcub::DeviceScan::InclusiveScan,
      input,
      output,
      scan_op,
      num_items,
  ...
  CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceScan::InclusiveScan,
      input,
      output,
      scan_op,
      size_cub,
  ...
  for (int64_t i = max_cub_size; i < num_items; i += max_cub_size) {
    ...
    impl::transform_vals<<<1, 1, 0, at::hip::getCurrentHIPStream()>>>(
        output + i - 1,
        input + i,
        first_elem_ptr,
        scan_op);
    C10_HIP_KERNEL_LAUNCH_CHECK();
#if !CUB_SUPPORTS_FUTURE_VALUE()
    using ArgIndexInputIterator = NO_ROCM(at_hip_detail)::hipcub::ArgIndexInputIterator<InputIteratorT>;
    ...
    auto input_ = NO_ROCM(at_hip_detail)::hipcub::TransformInputIterator<input_t, decltype(input_iter_transform), ArgIndexInputIterator>(
    ...
    CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceScan::InclusiveScan,
        ...
        at::hip::getCurrentHIPStream());
#else
    CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceScan::ExclusiveScan,
        input + i + 1,
        output + i,
        scan_op,
        ::at_hip_detail::hipcub::FutureValue<input_t>(first_elem_ptr),
        size_cub,
        at::hip::getCurrentHIPStream());
#endif
  }
...
inline void exclusive_scan(InputIteratorT input, OutputIteratorT output, ScanOpT scan_op, InitValueT init_value, int64_t num_items) {
#if defined(USE_ROCM) && (ROCM_VERSION >= 50000)
  //For ROCm, use hipCUB chained iterators
  CUB_WRAPPER(NO_ROCM(detail)::hiphipcub::DeviceScan::ExclusiveScan,
      input,
      output,
      scan_op,
      init_value,
  ...
  for (int64_t i = max_cub_size; i < num_items; i += max_cub_size) {
    ...
    CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceScan::ExclusiveScan,
        ...
        output + i,
        scan_op,
        ::at_hip_detail::hipcub::FutureValue<InitValueT>(first_elem_ptr),
        size_cub,
        at::hip::getCurrentHIPStream());
#endif
  }
...
inline void inclusive_sum_by_key(KeysInputIteratorT keys, ValuesInputIteratorT input, ValuesOutputIteratorT output, int64_t num_items) {
  ...
  CUB_WRAPPER(at_hip_detail::hipcub::DeviceScan::InclusiveSumByKey,
      keys, input, output, num_items, at_hip_detail::hipcub::Equality(), at::hip::getCurrentHIPStream());
}
...
inline void inclusive_scan_by_key(KeysInputIteratorT keys, ValuesInputIteratorT input, ValuesOutputIteratorT output, ScanOpT scan_op, int64_t num_items) {
  TORCH_CHECK(num_items <= std::numeric_limits<int>::max(),
    "cub InclusiveSumByKey does not support more than INT_MAX elements");
  CUB_WRAPPER(at_hip_detail::hipcub::DeviceScan::InclusiveScanByKey,
      keys, input, output, scan_op, num_items, at_hip_detail::hipcub::Equality(), at::hip::getCurrentHIPStream());
}
...
void unique(InputIteratorT input, OutputIteratorT output,
  ...
  CUB_WRAPPER(NO_ROCM(at_hip_detail)::hipcub::DeviceSelect::Unique,
              input, output, num_selected_out, num_items, at::hip::getCurrentHIPStream());
}
...
void run_length_encode(InputIteratorT input, OutputIteratorT output, CountsOutputIteratorT counts_out,
  ...
  CUB_WRAPPER(
      NO_ROCM(at_hip_detail)::hipcub::DeviceRunLengthEncode::Encode,
...
void reduce(InputIteratorT input, OutputIteratorT output, int64_t num_items, ReductionOpT op, T init) {
  ...
  CUB_WRAPPER(
      NO_ROCM(at_hip_detail)::hipcub::DeviceReduce::Reduce,
