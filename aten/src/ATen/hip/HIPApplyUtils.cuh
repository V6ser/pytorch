...
/*
  NOTE [ HIP_tensor_applyN helpers ]

  The following HIP_tensor_applyN (where N currently can be 1, 2, 3, or 4)
  ...
  HIP_tensor_apply2<scalar, scalar>(
    a, b,
  ...
  HIP_tensor_apply2<scalar1, scalar2, 2>(
    a, b,
...
//     HIP_tensor_applyN helpers. The input arg `n` to `apply()` represents the
...
//
// See NOTE [ HIP_tensor_applyN helpers ] above for how Op may look like.
...
template <typename scalar1, typename scalar2, int step, typename Op,
          int max_threads_per_block=AT_APPLY_THREADS_PER_BLOCK,
          int min_blocks_per_sm=AT_APPLY_BLOCKS_PER_SM>
inline bool HIP_tensor_apply2(at::TensorBase a,
                               at::TensorBase b,
                               const Op op,
                               TensorArgType aType = TensorArgType::ReadWrite,
                               TensorArgType bType = TensorArgType::ReadOnly) {
  TORCH_CHECK(a.device().is_hip() && b.device().is_hip(),
              "HIP_tensor_apply2: Expected tensors to have HIP DeviceType, but got "
...
#define HANDLE_CASE(TYPE, A, B)                                        \
  kernelPointwiseApply2<Op,                                            \
                        ...
                        min_blocks_per_sm>                             \
   <<<grid, block, 0, at::hip::getCurrentHIPStream(curDevice)>>>(    \
...
/* Provides default step = 1 to HIP_tensor_apply2. */
template <typename scalar1, typename scalar2, typename Op,
          int max_threads_per_block=AT_APPLY_THREADS_PER_BLOCK,
          int min_blocks_per_sm=AT_APPLY_BLOCKS_PER_SM>
inline bool HIP_tensor_apply2(const at::TensorBase &a,
                               const at::TensorBase &b,
                               const Op op,
                               TensorArgType aType = TensorArgType::ReadWrite,
                               TensorArgType bType = TensorArgType::ReadOnly) {
  return HIP_tensor_apply2<scalar1, scalar2, 1, Op,
