...
// Note:
// HIP and ROCm get diverged in this PR:
...
#if !defined(USE_ROCM)
  #include <ATen\native\hip\HIPLoops.cuh>
#else
  #include <ATen\native\hip\ROCmLoops.cuh>
#endif
...
void gpu_kernel_multiple_outputs(TensorIteratorBase& iter, const func_t& f) {
  ...
  for (int arg = 0; arg < iter.ntensors(); arg++) {
    TORCH_INTERNAL_ASSERT(iter.device(arg).is_hip());
  }
