...
__global__ void vol2im_kernel(
  ...
  HIP_KERNEL_LOOP(index, n) {
    ...
    const auto w_col_end = ::min(w_im / stride_w + 1, width_col);
    const auto h_col_start =
        (h_im < kernel_extent_h) ? 0 : (h_im - kernel_extent_h) / stride_h + 1;
    const auto h_col_end = ::min(h_im / stride_h + 1, height_col);
    const auto t_col_start =
        (t_im < kernel_extent_t) ? 0 : (t_im - kernel_extent_t) / stride_t + 1;
    const auto t_col_end = ::min(t_im / stride_t + 1, depth_col);
