...
constexpr inline integer get_log_num_threads_x_inner_scan(integer num_rows, integer row_size) {
  ...
  log_num_threads_x = ::min(::max((integer)4, log_num_threads_x), (integer)9);
...
__host__ void scan_outer_dim_with_indices(
  ...
  tensor_kernel_scan_outer_dim_with_indices<scalar_t><<<grid, threads, 0, at::hip::getCurrentHIPStream()>>>(
...
__host__ void scan_outer_dim(const TensorBase& self, const TensorBase& result,
  ...
  tensor_kernel_scan_outer_dim<scalar_t><<<grid, threads, 0, at::hip::getCurrentHIPStream()>>>(
...
void scan_dim(const TensorBase& self, const TensorBase& result,
  ...
  if (self.numel() == self.size(dim)) {
    cuda::hipcub::inclusive_scan(self_->const_data_ptr<scalar_t>(), result.mutable_data_ptr<scalar_t>(), binary_op, self.numel());
  } else if (dim == ndim - 1) {
