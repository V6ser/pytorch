...
#include <ATen\native\hip\MemoryAccess.cuh>

#include <ATen\native\hip\HIPJitLoops.cuh>
...
Loops.cuh (and HIPLoops.cuh) usually build. This reduces build time,
build size, and initial HIP context size.
...
operations instead of the typical HIP functors.
...
void jitted_gpu_kernel(
  ...
  for (int arg = 0; arg < iter.ntensors(); arg++) {
    TORCH_INTERNAL_ASSERT(
      iter.device(arg).is_hip(),
      "argument ", arg, ": expected a HIP device but found ", iter.device(arg));
  }
