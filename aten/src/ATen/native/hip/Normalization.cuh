...
#include <ATen\native\hip\block_reduce.cuh>
#include <ATen\native\hip\DeviceSqrt.cuh>
#include <ATen\native\hip\LaunchUtils.h>
...
__device__ scalar_t reduce(Op op, PTA tensor, int plane) {
  ...
  sum = hip_utils::BlockReduce<scalar_t, SumReduceOp<scalar_t>, hip_utils::Block2D>(sum, reduce_op, 0, shared);
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_backward_hip_template(const Tensor& grad_out_, const Tensor& input_, const Tensor& weight_,
...
void batch_norm_stats_hip_template(
...
template<typename input_scalar_t, typename stat_scalar_t, typename index_t>
void batch_norm_elemt_hip_template(const Tensor& output_, const Tensor& input_, const Tensor& weight_,
...
template<typename scalar_t, typename accscalar_t, typename index_t>
std::tuple<Tensor, Tensor> batch_norm_gather_stats_hip_template(const Tensor& mean_, const Tensor& invstd_,
...
template<typename input_scalar_t, typename stat_scalar_t, typename index_t>
std::tuple<Tensor, Tensor, Tensor, Tensor> batch_norm_backward_reduce_hip_template(const Tensor& grad_out_, const Tensor& input_,
...
template<typename input_scalar_t, typename stat_scalar_t, typename index_t>
Tensor batch_norm_backward_elemt_hip_template(const Tensor& grad_out_, const Tensor& input_,
...
void batch_norm_stats_channels_last_hip_template(
...
void batch_norm_elemt_channels_last_hip_template(
...
batch_norm_backward_reduce_hip_channels_last_template(const at::Tensor& grad_output,
    const at::Tensor& input,
    const at::Tensor& mean,
    const at::Tensor& inv_std,
    const at::Tensor& weight,
...
at::Tensor batch_norm_backward_elemt_channels_last_hip_template(
