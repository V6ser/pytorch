...
warpMergeSortKVInPlace(
  ...
  namespace cub = ROCM_HIPCUB(at_hip_detail::cub);
  ...
  using LoadKeys = hipcub::WarpLoad<K, items_per_thread, hipcub::WARP_LOAD_TRANSPOSE>;
  using LoadValues = hipcub::WarpLoad<V, items_per_thread, hipcub::WARP_LOAD_TRANSPOSE>;
  using Sort = hipcub::WarpMergeSort<K, items_per_thread, C10_WARP_SIZE, V>;
  using StoreKeys = hipcub::WarpStore<K, items_per_thread, hipcub::WARP_STORE_TRANSPOSE>;
  using StoreValues = hipcub::WarpStore<V, items_per_thread, hipcub::WARP_STORE_TRANSPOSE>;
...
radixSortKVInPlace(at::cuda::detail::TensorInfo<K, IndexType> keys,
  ...
  using key_t = typename at::cuda::hipcub::detail::hip_type<K>::type;
  ...
  using LoadValues = hipcub::BlockLoad<V, block_size, items_per_thread,
                                    hipcub::BlockLoadAlgorithm::BLOCK_LOAD_TRANSPOSE>;
  using Sort = hipcub::BlockRadixSort<key_t, block_size, items_per_thread, V>;
  ...
  const K invalid_key = [descending] {
    using radix_t = typename hipcub::Traits<key_t>::UnsignedBits;
    union {
      K key;
      radix_t radix;
    } tmp;
    tmp.radix = descending ?
        hipcub::Traits<key_t>::LOWEST_KEY :
        hipcub::Traits<key_t>::MAX_KEY;
    return tmp.key;
  }();
