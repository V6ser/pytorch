...
HIPAllocatorConfig::HIPAllocatorConfig()
    : m_max_split_size(std::numeric_limits<size_t>::max()),
      m_garbage_collection_threshold(0),
      m_pinned_num_register_threads(1),
      m_expandable_segments(false),
      m_release_lock_on_hipmalloc(false),
      m_pinned_use_hip_host_register(false) {
  m_roundup_power2_divisions.assign(kRoundUpPowerOfTwoIntervals, 0);
}

size_t HIPAllocatorConfig::roundup_power2_divisions(size_t size) {
...
void HIPAllocatorConfig::lexArgs(
    const char* env,
...
void HIPAllocatorConfig::consumeToken(
...
size_t HIPAllocatorConfig::parseMaxSplitSize(
    const std::vector<std::string>& config,
    size_t i) {
...
size_t HIPAllocatorConfig::parseGarbageCollectionThreshold(
    const std::vector<std::string>& config,
    size_t i) {
  consumeToken(config, ++i, ':');
  if (++i < config.size()) {
...
size_t HIPAllocatorConfig::parseRoundUpPower2Divisions(
    const std::vector<std::string>& config,
    size_t i) {
  consumeToken(config, ++i, ':');
...
size_t HIPAllocatorConfig::parseAllocatorConfig(
    const std::vector<std::string>& config,
    size_t i,
    bool& used_hipMallocAsync) {
  consumeToken(config, ++i, ':');
  if (++i < config.size()) {
    TORCH_CHECK(
        ((config[i] == "native") || (config[i] == "hipMallocAsync")),
        "Unknown allocator backend, "
        "options are native and hipMallocAsync");
    used_hipMallocAsync = (config[i] == "hipMallocAsync");
#ifndef USE_ROCM
    // HIP supports hipMallocAsync and does not need to check versions
    if (used_hipMallocAsync) {
...
void HIPAllocatorConfig::parseArgs(const char* env) {
  // If empty, set the default values
  m_max_split_size = std::numeric_limits<size_t>::max();
  m_roundup_power2_divisions.assign(kRoundUpPowerOfTwoIntervals, 0);
  m_garbage_collection_threshold = 0;
  bool used_hipMallocAsync = false;
  ...
  for (size_t i = 0; i < config.size(); i++) {
    if (config[i].compare("max_split_size_mb") == 0) {
      ...
      i = parseAllocatorConfig(config, i, used_hipMallocAsync);
      ...
      m_release_lock_on_hipmalloc = (config[i] == "True");
    } else if (
        // ROCm build's hipify step will change "cuda" to "hip", but for ease of
        // use, accept both. We must break up the string to prevent hipify here.
        ...
        config[i].compare("pinned_use_c"
                          "uda_host_register") == 0) {
      i = parsePinnedUseHipHostRegister(config, i);
      used_native_specific_option = true;
  ...
  if (used_hipMallocAsync && used_native_specific_option) {
    TORCH_WARN(
...
size_t HIPAllocatorConfig::parsePinnedUseHipHostRegister(
  ...
  if (++i < config.size()) {
    TORCH_CHECK(
        ...
        "Expected a single True/False argument for pinned_use_hip_host_register");
    m_pinned_use_hip_host_register = (config[i] == "True");
  } else {
    TORCH_CHECK(
        false, "Error, expecting pinned_use_hip_host_register value", "");
  }
  return i;
}
...
void setAllocatorSettings(const std::string& env) {
  HIPCachingAllocator::HIPAllocatorConfig::instance().parseArgs(env.c_str());
}
