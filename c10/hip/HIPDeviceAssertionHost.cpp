...
#define C10_HIP_CHECK_WO_DSA(EXPR)                                 \
  do {                                                              \
...
/// an infinite initialization loop for HIPKernelLaunchRegistry
int dsa_get_device_id() {
  int device = -1;
  C10_HIP_CHECK_WO_DSA(c10::hip::GetDevice(&device));
  return device;
}

/// Get a device's compute capability - note that this dangerously assumes
/// that if one HIP GPU supports device-side assertions they all do. This is
/// probably fine since the latest HIP GPU that doesn't support UVM is the
...
int dsa_get_device_compute_capability(const int device_num) {
  ...
  C10_HIP_CHECK_WO_DSA(hipDeviceGetAttribute(
      &compute_capability, hipDeviceAttributeComputeCapabilityMajor, device_num));
...
int dsa_get_device_count() {
  int device_count = -1;
  C10_HIP_CHECK_WO_DSA(c10::hip::GetDeviceCount(&device_count));
  return device_count;
}

bool dsa_check_if_all_devices_support_managed_memory() {
// It looks as though this'll work best on HIP GPUs with Pascal
...
void uvm_deleter(DeviceAssertionsData* uvm_assertions_ptr) {
  ...
  if (uvm_assertions_ptr) {
    C10_HIP_IGNORE_ERROR(hipFree(uvm_assertions_ptr));
  }
}
...
std::string c10_retrieve_device_side_assertion_info() {
#ifdef TORCH_USE_HIP_DSA
  const auto& launch_registry = HIPKernelLaunchRegistry::get_singleton_ref();
  ...
  for (const auto device_num : c10::irange(assertion_data.size())) {
    ...
    if (assertion_data_for_device.assertion_count >
      ...
      oss << "Adjust `C10_HIP_DSA_ASSERTION_COUNT` if you need more assertion failure info"
          << std::endl;
    }
    ...
    for (const auto i : c10::irange(failures_found)) {
      ...
      if (launch_info.generation_number == self.caller) {
        ...
        oss << "  CPU launch site info: Unavailable, the circular queue wrapped around. Increase `HIPKernelLaunchRegistry::max_size`."
            << std::endl;
      }
    }
  }
  return oss.str();
#else
  return "Compile with `TORCH_USE_HIP_DSA` to enable device-side assertions.\n";
#endif
}

HIPKernelLaunchRegistry::HIPKernelLaunchRegistry()
...
bool HIPKernelLaunchRegistry::check_env_for_enable_launch_stacktracing()
    const {
  return env_flag_set("PYTORCH_HIP_DSA_STACKTRACING");
}

bool HIPKernelLaunchRegistry::check_env_for_dsa_enabled() const {
  return env_flag_set("PYTORCH_USE_HIP_DSA");
}

uint32_t HIPKernelLaunchRegistry::insert(
...
std::pair<std::vector<DeviceAssertionsData>, std::vector<HIPKernelLaunchInfo>>
HIPKernelLaunchRegistry::snapshot() const {
...
DeviceAssertionsData* HIPKernelLaunchRegistry::
...
HIPKernelLaunchRegistry& HIPKernelLaunchRegistry::get_singleton_ref() {
  static HIPKernelLaunchRegistry launch_registry;
  return launch_registry;
}

bool HIPKernelLaunchRegistry::has_failed() const {
  for (const auto& x : uvm_assertions) {
