...
struct DeviceStats {
  ...
  // COUNT: total number of failed calls to HIP malloc necessitating cache
  // flushes.
...
struct TraceEntry {
  enum Action {
    ...
    SEGMENT_ALLOC, // a call to hipMalloc to get more memory from the OS
    SEGMENT_FREE, // a call to hipFree to return memory to the OS (e.g. to
...
class HIPAllocator : public Allocator {
 public:
  ...
  virtual void recordStream(const DataPtr&, HIPStream stream) = 0;
  ...
  // memory not allocated from hipMalloc cannot be copied
  // across devices using hipMemcpyAsync if peer to peer access is disabled.
  // instead it requires hipMemcpyAsyncPeer
  //  with P2P Enabled, all combinations work
  //  with P2P Disabled:
  //                       hipMalloc hipMallocAsync/cuMemMap
  // hipMemcpyAsyncPeer   works      works
  // hipMemcpyAsync       works      error
...
// See BackendInitializer in HIPCachingAllocator.cpp.
// Atomic loads on x86 are just normal loads,
// (atomic stores are different), so reading this value
// is no different than loading a pointer.
C10_HIP_API extern std::atomic<HIPAllocator*> allocator;

inline HIPAllocator* get() {
...
inline void recordStream(const DataPtr& dataPtr, HIPStream stream) {
...
// Not part of HIP_ALLOCATOR_BACKEND_INTERFACE
