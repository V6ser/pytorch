...
namespace hip::HIPCachingAllocator {

// Included here as this is externally used in HIPAllocatorConfig
 ...
 *
 * HIPGraph's requests for private pools are mediated by
...
using stream_set = ska::flat_hash_set<hip::HIPStream>;
...
For large (>2MB) allocations, the allocator calls hipMalloc to get allocations
...
hipMalloc a new (N + 1)*A*B segment. If there is not enough memory, there is
...
corresponding to empty physical pages, and return it to HIP for use at another
address in the segment or in a segment for a different stream.

A current limitation of HIP's API is that physical memory
...
will be slower than hipMalloc, though still in the milliseconds range for
...
* HIP runtime APIs related to sharing memory across process
(hipDeviceEnablePeerAccess) do not work for memory allocated with cuMemMap.
...
struct ExpandableSegment {
  ExpandableSegment(
    ...
    C10_HIP_CHECK(hipGetDeviceProperties(&prop, device_));
    // we allocate enough address space for 1 1/8 the total memory on the GPU.
    // This allows for some cases where we have to unmap pages earlier in the
    // segment to put them at the end.
    max_handles_ = numSegments(prop.totalGlobalMem + prop.totalGlobalMem / 8);
    C10_HIP_DRIVER_CHECK(DriverAPI::get()->cuMemAddressReserve_(
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      if (status == hipErrorMemoryAllocation) {
        for (auto j : c10::irange(begin, i)) {
          ...
          C10_HIP_DRIVER_CHECK(DriverAPI::get()->cuMemRelease_(h));
        }
        trimHandles();
        return rangeFromHandles(begin, begin);
      }
      C10_HIP_DRIVER_CHECK(status);
      handles_.at(i) = handle;
    }
    for (auto i : c10::irange(begin, end)) {
      C10_HIP_DRIVER_CHECK(DriverAPI::get()->cuMemMap_(
  ...
  ~ExpandableSegment() {
    ...
    C10_HIP_DRIVER_CHECK(DriverAPI::get()->cuMemAddressFree_(
 ...
 private:
  void setAccess(int device, size_t begin, size_t end) {
    ...
    C10_HIP_DRIVER_CHECK(DriverAPI::get()->cuMemSetAccess_(
        ptr_ + begin * segment_size_, (end - begin) * segment_size_, &desc, 1));
  }

  void unmapHandles(size_t begin, size_t end) {
    // note: unlike hipFree, MemUnmap and MemRelease do
    // not appear to synchronize in all cases, so we have to wait for the
    // stream to finish before this memory is truly free.

    // cannot call c10::hip::stream_synchronize because
    // it might grab the GIL which can lead to a deadlock
    // Locking order must be GIL -> Allocator Lock
    C10_HIP_CHECK(hipStreamSynchronize(stream_));
    for (auto i : c10::irange(begin, end)) {
      CUmemGenericAllocationHandle h = handles_.at(i).value();
      handles_.at(i) = c10::nullopt;
      C10_HIP_DRIVER_CHECK(DriverAPI::get()->cuMemUnmap_(
...
// Note: hipEventCreate when concurrently invoked from multiple threads can be
// very expensive (at least on certain device/driver combinations). Thus, we a)
// serialize event creation at a per-device level, and b) pool the events to
// avoid constantly calling hipEventCreate/hipEventDestroy. This results in
...
hipError_t hipMallocMaybeCapturing(void** p, size_t size) {
  ...
  if (at::cuda::currentStreamCaptureStatusMayInitCtx() ==
...
#endif
    return C10_HIP_ERROR_HANDLED(hipMalloc(p, size));
#if !defined(USE_ROCM) || ROCM_VERSION >= 50300
  } else {
    ...
    at::hip::HIPStreamCaptureModeGuard g{hipStreamCaptureModeRelaxed};
    return C10_HIP_ERROR_HANDLED(hipMalloc(p, size));
  }
#endif
}
...
static std::string reportProcessMemoryInfo(int device) {
  ...
  c10::call_once(nvml_init, [] {
    TORCH_INTERNAL_ASSERT(RSMI_STATUS_SUCCESS == DriverAPI::get()->nvmlInit_v2_());
  });
  ...
  std::stringstream ss;
  TORCH_INTERNAL_ASSERT(RSMI_STATUS_SUCCESS == r);
  ss << "";
...
class DeviceCachingAllocator {
 ...
 public:
  DeviceCachingAllocator()
    ...
    stats.max_split_size = HIPAllocatorConfig::max_split_size();
  ...
  Block* malloc(int device, size_t orig_size, hipStream_t stream) {
    ...
    if (C10_LIKELY(captures_underway.size() == 0)) {
      ...
      // A. process_events involves cudaEventQueries, illegal during HIP graph
      //    capture.
    ...
    if (!block_found) {
      ...
      if (C10_UNLIKELY(
              set_fraction &&
              HIPAllocatorConfig::garbage_collection_threshold() > 0.0)) {
    ...
    if (!block_found) {
      ...
      c10::reportOutOfMemoryToProfiler(
          ...
          stats.reserved_bytes[static_cast<int64_t>(StatType::AGGREGATE)]
              .current,
          c10::Device(c10::DeviceType::HIP, static_cast<DeviceIndex>(device)));
      ...
      TORCH_CHECK_WITH(
          ...
          " PYTORCH_HIP_ALLOC_CONF=expandable_segments:True to avoid"
  ...
  Block* alloc_found_block(
    ...
    if (block->size >= HIPAllocatorConfig::max_split_size())
    ...
    c10::reportMemoryUsageToProfiler(
        ...
        c10::Device(c10::DeviceType::HIP, device));
  ...
  void free(Block* block) {
    ...
    c10::reportMemoryUsageToProfiler(
        ...
        stats.allocated_bytes[static_cast<size_t>(StatType::AGGREGATE)].current,
        stats.reserved_bytes[static_cast<size_t>(StatType::AGGREGATE)].current,
        c10::Device(c10::DeviceType::HIP, block->device));
  }
  ...
  static size_t round_size(size_t size) {
    if (size < kMinBlockSize) {
      ...
      auto divisions = HIPAllocatorConfig::roundup_power2_divisions(size);
  ...
  // Called by HIPGraph::capture_begin
  ...
  // Called by HIPGraph::capture_end
  ...
  // Called by HIPGraph::reset
  void releasePool(MempoolId_t mempool_id) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    // The instantiated hipGraphExec_t has been destroyed. We can't blindly
 ...
 private:
  ...
  bool should_split(const Block* block, size_t size) {
    ...
    if (block->pool->is_small || HIPAllocatorConfig::expandable_segments()) {
      return remaining >= kMinBlockSize;
    } else {
      return (size < HIPAllocatorConfig::max_split_size()) &&
  ...
  bool get_free_block(AllocParams& p) {
    ...
    if ((*it)->expandable_segment_) {
      if (HIPAllocatorConfig::expandable_segments()) {
    ...
    if ((p.size() < HIPAllocatorConfig::max_split_size()) &&
        ((*it)->size >= HIPAllocatorConfig::max_split_size()))
      return false;
  ...
  void garbage_collect_cached_blocks(
    ...
    size_t gc_threshold = static_cast<size_t>(
        HIPAllocatorConfig::garbage_collection_threshold() *
  ...
  bool alloc_block(
    ...
    if (set_fraction &&
      ...
      return false;
        ...
        HIPAllocatorConfig::expandable_segments() &&
      ...
      if (HIPAllocatorConfig::release_lock_on_hipmalloc()) {
    ...
    if (size >= HIPAllocatorConfig::max_split_size())
  ...
  bool release_available_cached_blocks(
      const AllocParams& p,
      const std::shared_ptr<GatheredContext>& context) {
    if (HIPAllocatorConfig::max_split_size() ==
    ...
    key.size = (key.size < HIPAllocatorConfig::max_split_size())
        ? HIPAllocatorConfig::max_split_size()
        : key.size;
  ...
  void insert_events(Block* block) {
    int prev_device = 0;
    C10_HIP_CHECK(c10::hip::GetDevice(&prev_device));

    stream_set streams(std::move(block->stream_uses));
    AT_ASSERT(block->stream_uses.empty());
    for (auto& stream : streams) {
      C10_HIP_CHECK(c10::hip::SetDevice(stream.device_index()));

      EventPool::Event event =
          create_event_internal(static_cast<int>(stream.device_index()));
      C10_HIP_CHECK(hipEventRecord(*event, stream.stream()));

      block->event_count++;
      hip_events[stream].emplace_back(std::move(event), block);
    }

    C10_HIP_CHECK(c10::hip::MaybeSetDevice(prev_device));
  }
  ...
  void process_events(const std::shared_ptr<GatheredContext>& context) {
    ...
    for (auto it = hip_events.begin(); it != hip_events.end();) {
      ...
      while (!it->second.empty()) {
        ...
        hipError_t err = C10_HIP_ERROR_HANDLED(hipEventQuery(*event));
...
class NativeCachingAllocator : public HIPAllocator {
 private:
  std::mutex mutex;
 ...
 public:
  ...
  void setMemoryFraction(double fraction, int device) override {
    ...
    C10_HIP_CHECK(c10::hip::SetDevice(device));
  ...
  void recordStream(const DataPtr& ptr, hip::HIPStream stream) override {
  ...
  DataPtr allocate(size_t size) const override {
    ...
    HIPStream stream = hip::getCurrentHIPStream(device);
    ...
    if (forceUncachedAllocator()) {
      ...
      C10_HIP_CHECK(hipMalloc(&devPtr, size));
      const c10::impl::PyInterpreter* interp = c10::impl::GPUTrace::get_trace();
      if (C10_UNLIKELY(interp)) {
        (*interp)->trace_gpu_memory_allocation(
            reinterpret_cast<uintptr_t>(devPtr));
    ...
    return {devPtr, devPtr, deleteFunc, Device(DeviceType::HIP, device)};
  }
  ...
  void* raw_alloc(size_t nbytes) override {
    ...
    C10_HIP_CHECK(c10::hip::GetDevice(&device));
    void* r = nullptr;
    malloc(&r, device, nbytes, hip::getCurrentHIPStream(device));
    return r;
  }
  ...
  void enablePeerAccess(int dev, int dev_to_access) override {
    c10::hip::HIPGuard device_guard(dev);
  ...
  std::shared_ptr<void> getIpcDevPtr(std::string handle) override {
    ...
    int curr_device = 0;
    C10_HIP_CHECK(c10::hip::GetDevice(&curr_device));
    auto sp =
...
struct BackendStaticInitializer {
  ...
  // HIPAllocatorConfig. HIPAllocatorConfig double-checks it later (at
  // runtime). Defers verbose exceptions and error checks, including Hip
  // version checks, to HIPAllocatorConfig's runtime doublecheck. If this
  // works, maybe we should move all of HIPAllocatorConfig here?
  HIPAllocator* parseEnvForBackend() {
    const char* val = getenv("PYTORCH_HIP_ALLOC_CONF");
    if (val != nullptr) {
...
std::atomic<HIPAllocator*> allocator;
