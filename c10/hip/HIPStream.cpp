...
static std::atomic<uint32_t>
    priority_counters[c10::hip::max_compile_time_stream_priorities]
                     [C10_COMPILE_TIME_MAX_GPUS];

static hipStream_t streams[c10::hip::max_compile_time_stream_priorities]
...
static void initGlobalStreamState() {
  ...
  // HIP stream priorities are 1=low, 0=default, -1=high which differs from HIP
  ...
  max_stream_priorities = range >= c10::hip::max_compile_time_stream_priorities
      ? c10::hip::max_compile_time_stream_priorities
      : range;
}
...
static void initDeviceStreamState(DeviceIndex device_index) {
  ...
  for (const auto i : c10::irange(kStreamsPerPool)) {
    for (const auto p : c10::irange(max_stream_priorities)) {
      ...
      C10_HIP_CHECK(hipStreamCreateWithPriority(&stream, kDefaultFlags, pri));
...
HIPStream HIPStreamForId(DeviceIndex device_index, StreamId stream_id) {
  return HIPStream(
      HIPStream::UNCHECKED,
      Stream(
          Stream::UNSAFE,
          c10::Device(DeviceType::HIP, device_index),
          stream_id));
}
...
HIPStream getStreamFromPool(const int priority, DeviceIndex device_index) {
  initHIPStreamsOnce();
  if (device_index == -1) {
    device_index = current_device();
    c10::hip::SetTargetDevice();
  }
  TORCH_CHECK(
      priority <= 0,
  ...
  return HIPStreamForId(device_index, makeStreamId(id_type, idx));
}

HIPStream getStreamFromPool(const bool isHighPriority, DeviceIndex device) {
  initHIPStreamsOnce();
  int priority = isHighPriority ? -max_stream_priorities + 1 : 0;
  return getStreamFromPool(priority, device);
}

HIPStream getStreamFromExternal(
    hipStream_t ext_stream,
    DeviceIndex device_index) {
  // The stream pointer will be the actual id
  return HIPStreamForId(device_index, reinterpret_cast<int64_t>(ext_stream));
}
...
HIPStream getDefaultHIPStream(DeviceIndex device_index) {
  ...
  check_gpu(device_index);
  return HIPStreamForId(device_index, makeStreamId(StreamIdType::DEFAULT, 0));
}
...
HIPStream getCurrentHIPStream(DeviceIndex device_index) {
  ...
  check_gpu(device_index);
  return HIPStreamForId(device_index, current_streams[device_index]);
}
