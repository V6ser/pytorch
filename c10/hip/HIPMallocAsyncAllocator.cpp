...
namespace c10::hip::HIPCachingAllocator::HipMallocAsync {

#if TORCH_HIP_VERSION >= 11040
// HIP device allocator that uses hipMallocAsync to implement
// the same interface as HIPCachingAllocator.cpp.

// Designed to be safe for HIP graph capture.
// Interactions with HIP graph capture are mediated by
// notifyCaptureBegin
// notifyCaptureAboutToEnd
// notifyCaptureEnded
// notifyCaptureDestroy

// Implementation details, not declared in HIPCachingAllocator.h
namespace {
...
struct UsageStream {
  ...
  int device;
  UsageStream() = default;
  UsageStream(hipStream_t s, int d) : stream(s), device(d) {}
...
int device_count = 0;
// these don't need to be c10::once_flags as in HIPGeneratorImpl.cpp
...
/**
 * Note [Avoid freeing uncaptured ptrs during HIP graph capture]
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * During HIP graph capture, it's illegal to call hipFreeAsync
...
/**
 * Note [Avoid dangling free streams during HIP graph capture]
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
inline void lazy_init_device(int device) {
  if (!devs_initialized_flags[device]) {
    ...
    C10_HIP_CHECK(cudaDeviceGetDefaultMemPool(&mempool, device));
    uint64_t threshold = UINT64_MAX;
    C10_HIP_CHECK(cudaMemPoolSetAttribute(
...
inline void sync_raw(hipStream_t dependency, hipStream_t dependent) {
  // HIPCachingAllocator.cpp uses raw cuda events, as do we.
  hipEvent_t event = nullptr;
  C10_HIP_CHECK(hipEventCreateWithFlags(&event, hipEventDisableTiming));
  C10_HIP_CHECK(hipEventRecord(event, dependency));
  C10_HIP_CHECK(hipStreamWaitEvent(dependent, event));
...
inline void free_impl(PtrInfo::iterator& it) {
  ...
  HIPGuard g(creation_stream.device);
  ...
  if (recorded_streams.empty()) {
    ...
    C10_HIP_CHECK(hipFreeAsync(it->first, creation_stream.stream));

    if (C10_UNLIKELY(capture_underway)) {
      // See Note [Avoid dangling free streams during HIP graph capture]
    ...
    for (const auto& recorded_stream : recorded_streams) {
      ...
      // same device.
      HIPGuard g_usage(recorded_stream.device);

      sync_raw(recorded_stream.stream, dummy_unifying_free_stream.stream);
    }

    // Frees ptr in the dummy "unifier" stream.
    C10_HIP_CHECK(hipFreeAsync(it->first, dummy_unifying_free_stream.stream));
...
void freeAsync(void* ptr) {
  ...
  if (C10_UNLIKELY(capture_underway)) {
    if (!it->second.captured) {
      ...
      // See Note [Avoid freeing uncaptured ptrs during HIP graph capture]
...
void mallocAsync(void** devPtr, int device, size_t size, hipStream_t stream) {
  TORCH_INTERNAL_ASSERT(
...
// Same pattern as HIPCachingAllocator.cpp.
struct HipMallocAsyncAllocator : public HIPAllocator {
  DataPtr allocate(size_t size) const override {
    ...
    void* r = nullptr;
    if (size != 0) {
      mallocAsync(&r, device, size, hip::getCurrentHIPStream(device));
    }
    return {r, r, &local_raw_delete, Device(DeviceType::HIP, device)};
  }
  ...
  void cacheInfo(int device, size_t* maxWorkspaceGuess) override {
    ...
    // maximum workspace size to use for an upcoming miopenFind call.
    //
    ...
    // perf-inefficient because cacheInfo is a prelude to miopenFind.
    //
    ...
    // the bindings rerun miopenFind, including calling cacheInfo again
    ...
    size_t device_total = 0;
    C10_HIP_CHECK(hipMemGetInfo(&free_upper_bound, &device_total));
    TORCH_INTERNAL_ASSERT(
  ...
  DeviceStats getDeviceStats(int device) override {
    ...
    if (devs_initialized_flags[device]) {
      ...
      cudaMemPool_t mempool = nullptr;
      C10_HIP_CHECK(cudaDeviceGetDefaultMemPool(&mempool, device));
      C10_HIP_CHECK(cudaMemPoolGetAttribute(
  ...
  void resetPeakStats(int device) override {
    ...
    cudaMemPool_t mempool = nullptr;
    C10_HIP_CHECK(cudaDeviceGetDefaultMemPool(&mempool, device));
    // Using zero as the reset value is the method recommended by Hip driver
  ...
  void endAllocateToPool(int device, MempoolId_t mempool_id) override {
    ...
    auto capture_stream = hip::getCurrentHIPStream(device);

    // See Note [Avoid dangling free streams during HIP graph capture]
    for (const auto& free_stream : capture_free_streams) {
      // hipEventRecord requires that the input event and stream are on the
      // same device.
      HIPGuard g(free_stream.device);

      // HIPCachingAllocator.cpp uses raw cuda events, as do we.
      hipEvent_t event = nullptr;
      C10_HIP_CHECK(hipEventCreateWithFlags(&event, hipEventDisableTiming));
      C10_HIP_CHECK(hipEventRecord(event, free_stream.stream));
      C10_HIP_CHECK(hipStreamWaitEvent(capture_stream.stream(), event));
      C10_HIP_CHECK(hipEventDestroy(event));
    }
  ...
  void* raw_alloc(size_t nbytes) override {
    ...
    C10_HIP_CHECK(c10::hip::GetDevice(&device));
    void* r = nullptr;
    mallocAsync(&r, device, nbytes, hip::getCurrentHIPStream(device));
    return r;
  }
  ...
  void enablePeerAccess(int dev, int dev_to_access) override {
    ...
    cudaMemPool_t mempool = nullptr;
    C10_HIP_CHECK(cudaDeviceGetDefaultMemPool(&mempool, dev_to_access));
    cudaMemAccessDesc desc = {};
    desc.location.type = cudaMemLocationTypeDevice;
    desc.location.id = dev;
    desc.flags = cudaMemAccessFlagsProtReadWrite;
    C10_HIP_CHECK(cudaMemPoolSetAccess(mempool, &desc, 1 /* numDescs */));
  }
...
HipMallocAsyncAllocator device_allocator;
...
#else
HIPAllocator* allocator() {
  TORCH_CHECK(false, "Cannot use hipMallocAsyncAllocator with cuda < 11.4.");
  return nullptr;
}
